...
const uploadFile = async (file: File): Promise<string | null> => {
  ...
  try {
    const res = await fetch('https://api.cloudinary.com/v1_1/dhg7c7ypc/upload', {
      method: 'POST',
      body: data,
    });
...
export default function ContactForm() {
  ...
  return (
    <section id="contact" className="min-h-screen w-full flex flex-col md:flex-row p-4 pt-8 md:p-15 fontBody justify-center md:justify-normal">
      <div
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col space-y-6 sm:space-y-8">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2, duration: 0.6 }}>
              <p className="text-xs sm:text-sm mb-2 sm:mb-3 text-gray-400">I'm interested in...</p>
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {services.map((service, i) => (
                  <motion.button
                    ...
                    className={`px-3 sm:px-4 py-1 sm:py-2 border rounded-md text-xs sm:text-sm ${selected === service ? 'bg-white text-black' : 'border-gray-600 text-gray-300 hover:border-white'}`}
                  >
                    {service}
                  </motion.button>
                ))}
            ...
            >
              <label htmlFor="attachment" className="flex items-center gap-2 cursor-pointer hover:text-white">
              ...
              {formData.attachment && <span className="text-xs ml-2 text-gray-500">{formData.attachment.name}</span>}
            </motion.div>
